...
def test_operation(operation_name, operation_function):
    try:
        # Cek apakah hasil sudah tersedia di Memcached
        cached_result = memcache_client.get(operation_name)
        if cached_result is not None:
            start_time = time.time()
            print(f"Hasil {operation_name} didapat dari Memcached.")
            end_time = time.time()
        ...
        if db is not None:
            ...
            # Simpan hasil eksekusi ke Memcached
            memcache_client.set(operation_name, execution_time)
        else:
...
save_to_file('hasil/results.txt', 'Komparasi Mongo DB dengan Caching')
